# AI 圓桌 v3.10.0｜Cloudflare 自動切 Gemini 3.5 備援版
...
- v3.10.0：
  - **Web Search 自動觸發**：這題如果講得夠明確是要查最新資訊（例如「上網查」「最新版本」「即時股價」這類字眼），
    即使忘記手動打開 Web Search 開關，也會自動幫這一題開啟——只作用在那一題，不會偷偷把開關永久打開，
    畫面上會有一張卡片告訴你「這題自動開了」，不是悄悄執行。
