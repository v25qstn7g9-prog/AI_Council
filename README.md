...
- 頁首可選主 AI、備援 AI，並一鍵互換；選擇保存在目前瀏覽器。聊天、附件工程、GitHub 工程均帶入相同路由設定。
- `/models` 只回傳模型名稱與是否已設定，不回傳 API key；「已設定」不代表服務健康或仍有可用額度。
- 自動模式保留原本 A/B/C 模型分工；手動指定主服務時，各角色使用該服務設定的模型，不代表三個不同模型。
- 主服务呼叫失敗會嘗試可用備援；同次請求避開已失敗服務，下次請求重新嘗試主服務。必須設定至少兩個服務才能跨服務備援。
- 聊天提高主答／複核輸出上限至 1800／1400 tokens，保留最近 18 則歷史；較長回答與備援呼叫可能增加用量。複核失敗仍保留主答。
- 深藍金色工作台、可收合工具設定與長回答、複製回答、手機響應式排版；模型能力與可用性仍取決於管理者設定及供應商額度。
- 執行 `npm test` 驗證原有審核流程與主備援路由的 mock 回歸測試；不會呼叫付費模型。
